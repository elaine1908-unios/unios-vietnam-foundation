...
export function loadDetail(id: string) {
  const row = db
    .prepare(
      `SELECT jd.id, jd.location, jd.is_archived, jd.is_now_hiring, jd.created_at, jd.updated_at, jd.job_profile_id,
              jp.job_title, jp.division, jp.function
...
jobDescriptionsRouter.get("/", (req, res) => {
  ...
  const rows = db
    .prepare(
      `SELECT jd.id, jd.location, jd.is_archived, jd.is_now_hiring, jd.updated_at, jp.job_title, jp.division, jp.function
       FROM job_descriptions jd JOIN job_profiles jp ON jp.id = jd.job_profile_id
...
jobDescriptionsRouter.post("/", requireTeamLead, (req, res) => {
  ...
  const id = newId();
  db.prepare(
    "INSERT INTO job_descriptions (id, job_profile_id, location, is_now_hiring, created_by, updated_by) VALUES (?, ?, ?, ?, ?, ?)",
  ).run(id, input.job_profile_id!.trim(), input.location!.trim(), input.is_now_hiring ? 1 : 0, req.user!.id, req.user!.id);
