-- Code of Conduct: one shared document version (editing any section bumps
-- it for all of them, per the source PDF's own single-version-for-the-
-- whole-document convention), split into named sections each editable
-- independently. code_of_conduct_document is a singleton — always exactly
-- one row.
CREATE TABLE code_of_conduct_document (
  id TEXT PRIMARY KEY,
...
CREATE TABLE code_of_conduct_sections (
  id TEXT PRIMARY KEY,
  sort_order INTEGER NOT NULL,
  title TEXT NOT NULL,
  -- Sanitized HTML from the rich text editor (see routes/codeOfConduct.ts —
  -- sanitized again server-side with isomorphic-dompurify before storage,
  -- not just on render, so a stored row is always safe to render as-is).
  content TEXT NOT NULL DEFAULT '',
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_at TEXT NOT NULL DEFAULT (datetime('now')),
  updated_by TEXT REFERENCES users(id)
);
CREATE INDEX idx_code_of_conduct_sections_sort ON code_of_conduct_sections(sort_order);

-- Coarser than audit_log (which still gets one row per section edit, same
-- as everything else in this app) — this is specifically "the document
-- went from version X to version Y on this date", the history a reader
-- skims, not a field-level diff.
CREATE TABLE code_of_conduct_version_history (
  id TEXT PRIMARY KEY,
  version TEXT NOT NULL,
  version_date TEXT NOT NULL,
...
-- Seed content extracted from UV_Code of conduct_2.6_21042026.pdf
INSERT INTO code_of_conduct_document (id, version, version_date) VALUES ('7a280b44-bdbe-4f57-a5cd-c0c82f1a4f13', '2.6', '2026-04-21');

INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('439ee39f-28ae-4e0a-afae-824cfc61cc0e', 1, 'Introduction', '<h4>1.1 Lời nói đầu</h4>
...
<h4>1.4 General</h4>
<p>Amendments to this Code of Conduct will be issued from time to time. It does not form part of your contract of employment, unless expressly stated otherwise. However, in any event, the Code of Conduct may be considered when interpreting your rights and obligations under your terms of employment. Date / Ngày Version Signature / Ký tên</p>');
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('ace3a56a-dc55-4805-aaac-76047da7b429', 2, 'Joining the Organisation', '<h4>2.1 Thời gian thử việc</h4>
...
<p>Trong khoảng thời gian làm việc tại công ty, bạn được yêu cầu báo cáo ngay lập tức về những hành vi vi phạm pháp luật mà bạn đã hoặc đang có thể bị kết án hoặc buộc tội.</p>
...
<p>During your employment, you are required to immediately report to the Employer any convictions or offences with which you may be potentially or have been charged.</p>');
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('884a0a07-4b4d-4d1c-ae3f-ec37ecbffa61', 3, 'Remuneration and Hours', '<h4>3.1 Giờ làm việc</h4>
...
<p>Công ty sẽ đóng cho bạn các loại bảo hiểm như Bảo hiểm xã hội, Bảo hiểm y tế và Bảo hiểm thất nghiệp theo quy định của Bộ luật Lao động Việt Nam. Mức đóng bảo hiểm sẽ được căn cứ vào mức lương cơ bản được quy định trong hợp đồng lao động và thoả thuận của bạn đối với công ty.</p>
...
<p>The company shall pay the insurance premium for employees under the Vietnam Labour Code. This includes Social insurance, Health insurance and Unemployment insurance and shall be calculated according to the basic salary specified in the Labour contract and the agreement between you and the Employer.</p>');
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('8903026f-dc5b-46bc-a5db-17a711ce0c87', 4, 'Leave Management', '<h4>4.1 Điều khoản chung</h4>
...
<p>software two (02) weeks in advance. You are encouraged to use up these days off throughout the year. This leave will not be cumulated to the subsequent và ngày phép này sẽ không được cộng dồn cho các năm tiếp theo. Trong trường hợp đến hết tháng 12 mà bạn vẫn còn ngày phép chưa sử dụng, bạn sẽ được nhận thanh toán chi phí ngày phép dư trong đợt thanh toán lương cuối cùng của năm. Mức thanh toán được tính như sau: Tổng lương căn bản trong năm / Tổng số ngày công trong năm * Số ngày nghỉ phép còn lại Tuỳ thuộc vào tình hình nhân sự và khối lượng công việc, số lượng đơn nghỉ phép được duyệt sẽ bị hạn chế trong các khung thời gian sau:</p>
...
<p>Theo luật lao động hiện hành và nội quy của công ty, bạn được quyền xin nghỉ phép để tổ chức hôn lễ theo quy định sau: Nghỉ phép được hưởng lương ba (03) ngày khi:</p>
...
<p>calendar) The specific schedule will be notified to you in official writing. If any of the above public holidays fall on a weekly rest day, the Employee shall be entitled to a compensatory day off on the next working day. In case you are required to go to work on this holiday (except for the Christmas day), the company will pay you at least 400% in total.</p>');
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('6fa5a18e-8f07-41e8-adca-a81a34ce5305', 5, 'Safeguards', '<h4>5.1 Quyền lục soát</h4>
...
<p>ra ngoài mà chưa được sự đồng ý của phòng ban liên quan</p>
...
<p>The use of the work email system (work email) is encouraged as its appropriate use facilitates efficiency. Used correctly, it is a facility that is of assistance to the Employer. However, inappropriate use causes a number of problems, including distractions, time wasting and legal claims. Unauthorised or inappropriate use of work email may result in disciplinary action. Email công ty luôn sẵn sàng để bạn liên lạc với đồng nghiệp, khách hàng và những mục đích khác liên quan đến hoạt động kinh doanh hợp pháp của doanh nghiệp. Nhân viên sử dụng email công ty nên:</p>
...
<p>Hệ thống giám sát có thể hoạt động bất kỳ lúc nào, liên tục hoặc gián đoạn tuỳ thuộc vào quyết định của ban quản lý. Công ty có quyền tiết lộ hồ sơ giám sát với các bên liên quan miễn là nó không vi phạm đến quyền riêng tư. Bạn có thể tham khảo ý kiến của công ty về bất kỳ mối quan tâm nào đến việc giám sát này. Tất cả các camera và các thiết bị ghi âm sẽ không được đặt ở khu vực phòng tắm và phòng thay đồ. Mục đích của hệ thống giám sát này là đảm bảo an toàn và Employer’s best interests at all times. All employees are prohibited from using personal social media (whether on the Employer’s devices or their own personal device) during work time for personal reasons. Any breach of this policy will be considered serious and may result in disciplinary action.</p>
...
<p>ty bị hư hỏng, bạn cần nộp lại phôi cho công ty để được đổi mới. Nếu bạn đến công ty với trang phục không phù hợp, quản lý có quyền yêu cầu bạn về thay và thời gian đi làm trễ sẽ không được tính vào lương. Bất kỳ hành động vi phạm liên tục hoặc cố ý sẽ dẫn đến biện pháp kỷ luật thích hợp. Nếu có thắc mắc về trang phục, bạn có thể tham khảo ý kiến của người quản lý. Trong trường hợp công ty từ chối ký hợp đồng tiếp tục hoặc bạn đơn phương chấm dứt hợp đồng với Unios, bạn được yêu cầu nộp lại tất cả các tài sản mang thương hiệu Unios. issued for your protection because of the nature of your job and if issued must be worn and used at all appropriate times. Failure to do so could be a contravention of your health and safety responsibilities. Specifically, you must ensure that you comply with the following requirements:</p>
...
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('d2266c53-379a-4617-ac14-62cac6fa4369', 8, 'Disciplinary', '<h4>8.1 Lời nói đầu</h4>
...
<ul><li>Identification and documentation of the violation</li><li>Notification to the employee</li><li>Tạo cơ hội để người lao động giải trình</li><li>Xem xét, đánh giá bởi quản lý và/hoặc bộ phận Nhân</li></ul>
<p>sự</p>
...
<ul><li>Repeated failure to meet reporting or KPI requirements</li><li>Nghỉ không phép (tối đa 1.5 ngày/tháng)</li><li>Không chấp hành chỉ đạo từ quản lý</li><li>Sử dụng thời gian làm việc cho việc cá nhân mà không</li></ul>
<p>được phép</p>
...
<ul><li>Mức độ nghiêm trọng và ảnh hưởng thực tế có thể được</li><li>Unauthorized absence (up to 1.5 days/month)</li><li>Failure to follow instructions from management</li><li>Use of working hours for personal matters without</li></ul>
<p>approval</p>
...
<p>Offence / Mức sai phạm 1st occasion /  Lần 1 2nd occasion / Lần 2 3rd occasion / Lần 3 Level 1 Verbal warning Cảnh cáo miệng Written warning Cảnh cáo văn bản Final written warning Cảnh cáo văn bản cuối cùng Level 2 Written warning Cảnh cáo văn bản Final written warning Cảnh cáo văn bản cuối cùng Demotion/Position removal Cách chức/Giáng chức Level 3 Demotion/Position removal Cách chức/Giáng chức Termination Kỷ luật sa thải Level 4 Termination Kỷ luật sa thải xem xét để điều chỉnh hình thức kỷ luật</p>
...
<h4>10.1 General requirement</h4>
...
<h4>11.3 Hygiene</h4>
<p>Any exposed cut or burn must be covered with a first-aid dressing. If you are suffering from an infectious or contagious disease or illness such as rubella or hepatitis you must not enter the workplace without clearance from your own doctor. Contact with any person suffering from an infectious or contagious disease must be reported before commencing work. If you go to work with poor health conditions that might be a risk to the community, company will send you home for an unpaid rest.</p>');
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('8aeed9b2-2e12-4992-b056-7bb41a79cfdb', 12, 'Office & Showroom Policy', '<h4>12.1 Khu vực văn phòng</h4>
<p>Bạn cần đảm bảo:</p>
...
<p>mà không được sự đồng ý từ Showroom Manager hoặc Operation.</p>
<ul><li>Các case mẫu sau khi đi tiếp khách về cần để gọn gàng</li></ul>
<p>ở khu vực được sắp xếp từ trước.</p>
...
<p>arranged area after returning the office</p>');
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('a3f10e55-7899-40d3-95ac-48d450922444', 13, 'Bullying & Harassment', '<h4>13.1 Giới thiệu chung</h4>
...
<p>comments</p>
<ul><li>Unjustified criticism or complaints</li><li>Các đe doạ về cả thể chất lẫn tinh thần</li><li>Loại trừ ra khỏi các hoạt động đội, nhóm</li><li>Truyền bá thông tin không chính xác và tin đồn sai</li></ul>
<p>lệch</p>
...
<h4>14.1 Illicit drugs and alcohol</h4>
...
<p>You are required to smoke only in designated smoking areas within the building or showroom. If you are working on a project site or any location outside the company premises, you must comply with the smoking regulations of that location. In addition, the company strictly prohibits smoking— including electronic cigarettes (e-cigarettes/vaping devices)—in the office, in company-owned vehicles, and in any areas where smoking is prohibited for occupational health and safety reasons.</p>');
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('0c11fb9d-0974-4d15-8713-58dbae9f21e5', 15, 'Whistle-Blowers', '<p>Nếu bạn tin rằng công ty hoặc bất kỳ cá nhân nào đang làm việc tại công ty có liên quan đến bất kỳ hình thức sai phạm nào bao gồm :</p>
...
<p>xác trong thời hạn 30 ngày hoặc sớm hơn. Trong trường hợp bạn không thể hoàn thành đầy đủ các biên bản bàn giao này trong thời hạn 30 ngày, công ty sẽ yêu cầu bạn chi trả một số tiền để bổ sung cho sự thiếu trách nhiệm trong công việc của bạn. Khoản tiền này có thể sẽ bị khấu trừ trong khoản thanh toán công ty chưa chi trả cho bạn. All handover forms should be completed correctly within 30 days or earlier. In the event that you are unable to complete these handover checklists within 30 days, the company will ask you to pay an amount of penalty to fill the lack of responsibility in your work. This amount may be deducted from any payments the company pays you.</p>');
INSERT INTO code_of_conduct_sections (id, sort_order, title, content) VALUES ('751e22f2-101e-4f62-988c-9a5a1211a39a', 17, 'Acknowledgement Form', '<p>I hereby acknowledge that I have read, understood, and agree to comply with this Code of Conduct . I undertake to review and update my understanding of this document as required by Unios Vietnam. Tôi xác nhận rằng tôi đã đọc, hiểu và đồng ý tuân thủ Bộ Quy tắc Ứng xử này. Tôi cam kết sẽ thường xuyên cập nhật và tuân thủ các nội dung của tài liệu theo yêu cầu của Unios Vietnam. Signature Date</p>');

INSERT INTO code_of_conduct_version_history (id, version, version_date, section_title) VALUES ('e3faac5b-b5fe-42c9-b744-9cf37d61c4ae', '2.6', '2026-04-21', 'Initial import from UV_Code of conduct_2.6_21042026.pdf');
